...
@section('headtext','LATIHAN SOAL JAVASCRIPT')
    ...
    <div class="container d-flex flex-column">
        <div id="question">
            ...
            <form action="/latihan/js/{{$mc->id}}" method="post">
                @csrf
                @method('patch')
                @foreach ($multiplechoice as $mc)    
                <div class="row">
                    ...
                    <nav aria-label="...">
                        <ul class="pagination pagination-lg flex-wrap">
                            ...
                            @foreach ($paginatesoal as $re)
                                <li class="page-item py-2 px-2" ><a style="width: 60px;" class="page-link @if($re->is_act == 1 ) is_act @endif @if(Request::fullUrl() == 'http://127.0.0.1:8000/latihan/js?page='.$re->id ) no_act @endif" href="{{url('/latihan/js/?page='.$re->id)}}">{{$re->id}}</a></li>
                            @endforeach
                        </ul>
                      </nav>
                </div>  
                <div class="row my-4 ">
                    <div class="col-md d-flex justify-content-start">
                        ...
                        @if (Request::fullUrl() == 'http://127.0.0.1:8000/latihan/js?page='.$paginatesoal->last()->id)
                            <a class="btn btn-primary soal" href="{{route('recapjs')}}">Selesai Latihan</a>
                        @endif
